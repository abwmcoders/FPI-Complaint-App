...
import 'Components/ComplaintDetails.dart';
...
class _DashboardScreenState extends State<DashboardScreen> {
    ...
    await firestore.collection("User").doc(user?.email).get().then((value) {
      if (mounted) {
        setState(() {
          ...
          userAccountType = userObject.accountType!;
          currentUserEmail = userObject.email!;
          currentUserName = userObject.name!;
          currentUserPass = userObject.pass!;
  ...
  Future<void> StreamListener() async {
    firestore
        ...
        .where('username',
            isEqualTo:
                user?.email.toString().substring(0, user?.email!.indexOf('@')))
        .snapshots()
        .listen((event) {
      Pending = event.docs
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        centerTitle: true,
        title: const Text("FPI Complaints App"),
        actions: [
                        ...
                        children: [
                          ...
                          Card(
                            ...
                            child: Container(
                              ...
                              height: userAccountType == "Student" ? 200 : 500,
                              ...
                              child: StreamBuilder<QuerySnapshot?>(
                                stream: firestore
                                    ...
                                    .where('username',
                                        isEqualTo:
                                            context.read<Info>().user.name)
                                    .orderBy("timestamp", descending: true)
                                    .limit(userAccountType == "Student" ? 3 :10)
                                    .snapshots(),
