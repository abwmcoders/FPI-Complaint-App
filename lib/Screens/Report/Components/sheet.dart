...
class _SheetState extends State<Sheet> {
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      //         serviceItem.add(
                      //           DropdownMenuItem(
                      //             child: Text(element.get('manager')),
                      //             value: element.get('manager'),
                      //           ),
                      //         );
                      //       }
                      //       return DropdownButtonHideUnderline(
                  ...
                  Container(
                      ...
                      child: ElevatedButton(
                          onPressed: UpdateReport, child: const Text("Update"))),
                ],
              ),
            ),
          ),
  ...
  UpdateReport() async {
    ...
    firestore.collection("Complains").doc(widget.id).update({
      ...
      Utils.showSnackBar("Updated Successfully", Colors.green);
        ...
        Utils.showSnackBar("Failed to update user: $onError", Colors.red));

    Provider.of<ComplaintObject>(context, listen: false)
        .setService(selectedService.toString());
    // Provider.of<ComplaintObject>(context, listen: false)
    //     .setWorker(selectedWorker.toString());
    Provider.of<ComplaintObject>(context, listen: false)
        .setStatus(selectedStatus.toString());
    Provider.of<ComplaintObject>(context, listen: false)
        .setPriority(selectedPriority.toString());

    String token = await Token.CheckToken(
        context.read<ComplaintObject>().complaint.username.toString() +
            "@berry.edu.pk");
    if (token != "false") {
      NotifyUser.sendPushMessage(
        token,
        "Title: " +
            context.read<ComplaintObject>().complaint.title.toString() +
            "   Service: " +
  ...
  //     firestore
  //         .collection("Worker")
  //         .doc(selectedWorker.toString() + "@berry.edu.pk")
  //         .update({
